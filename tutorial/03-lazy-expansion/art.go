// Package lazyexpansion is chapter 3 of the literate tutorial. It
// keeps chapter 2's full-fanout 256-child inner node but introduces
...
// (or a tree root) can hold either kind. Chapter 6 will give that
...
// each remains. That is what path compression (chapter 4) removes.
...
// *node256[V]. The interface is deliberately empty: chapter 6
...
// node256. Chapter 4 collapses this chain into a single prefix.
...
//
// The recursion has two new collapse cases compared with chapter 2:
//
...
// yields every entry.
//
// The pleasant change from chapter 2: leaves carry the full key, so
