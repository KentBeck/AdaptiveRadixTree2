...
func reportFootprint[V any](b *testing.B, t *Tree[V], nKeys int) {
	...
	b.ReportMetric(float64(bytes)/float64(nKeys), "stage2-bytes/key")
	b.ReportMetric(float64(t.CountNodes())/float64(nKeys), "stage2-nodes/key")
}
...
func runRangeBenches(b *testing.B, w bench.Workload) {
	...
	b.Run("Stage2", func(b *testing.B) {
		...
		var sink int
		for i := 0; i < b.N; i++ {
			for _, v := range stage2.Range(nil, nil) {
				sink ^= v
			}
		}
		_ = sink
...
// via AscendRange; for chapter 2's ART it is Range(lo, hi). The
...
func runRangeWindowBenches(b *testing.B, w bench.Workload) {
	...
	for k := range stage2.Range(nil, nil) {
	...
	b.Run("Stage2", func(b *testing.B) {
		...
		var sink int
		for i := 0; i < b.N; i++ {
			for _, v := range stage2.Range(lo, hi) {
				sink ^= v
			}
		}
		_ = sink
