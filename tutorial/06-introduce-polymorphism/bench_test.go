...
import (
	...
	stage4 "github.com/KentBeck/AdaptiveRadixTree2/tutorial/05-add-node4"
...
// is the price of the refactor; chapter 6's prose discusses it
...
// number should match chapter 5's exactly times the small per-node
...
// runRangeWindowBenches measures iteration of the *middle* 1 %
// of the sorted keys: a window from the 49.5th to the 50.5th
// percentile of the keyspace. For btree this is a sub-range walk
// via AscendRange; for both stage 4 and stage 5's ART it is
// Range(lo, hi). Range still walks every leaf and filters at the
// yield site, so the cost is dominated by the unavoidable full
// descent; chapter 9 will prune subtrees by prefix and close the
// gap.
