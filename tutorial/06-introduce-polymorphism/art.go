// Package introducepolymorphism is chapter 6 of the literate
...
// bench panel reports "essentially unchanged" numbers vs chapter 5,
// and that's the point: the refactor is not a performance play.
// Its return-on-investment shows up in chapter 7 (adding node16) and
// chapter 8 (adding node48), where each new node type lands as a
...
//   - chapter 5's nine free functions, each two cases of a type
//     switch, vs chapter 6's two method sets (one per node type)
//   - chapter 5's Put / Get / Delete bodies threading through
//     `nodePrefix[V](current)`-style helpers, vs chapter 6's
//     `n.getPrefix()`-style direct method calls
//   - chapter 5's `reshape` -- a free function with embedded
//     type-specific logic -- vs chapter 6's `(*node4).reshape()`
//     and `(*node256).reshape()` methods, each holding only its
//     own logic
//
// The production `art.Tree` uses the same shape; chapter 9 will
...
// type switch in chapter 5 now goes through one of these methods.
//
...
// interface stays narrower; chapter 7 / 8's drop-ins are simpler.
...
// node4 is the small inner node from chapter 5. Field types:
...
// type-specific logic that used to live in chapter 5's free function
...
// every leaf and filters bounds at the yield site; chapter 9 will
