// Package pathcompression is chapter 4 of the literate tutorial. It
...
//
// Lazy expansion (chapter 3) eliminated the chain of one-child
...
// The headline win is on URL workloads: ~1 800 B/key in chapter 3
...
// node is the same sum-type marker as chapter 3.
type node interface {
	isNode()
}
...
//
// The new wrinkle vs chapter 3 is the prefix-match step on every
...
// that share a (possibly empty) prefix. Compared with chapter 3's
...
// (chapter 3) this collapse was impossible because there was
...
// yields every entry.
//
// Same walk shape as chapter 3: iterate the terminal first (its
...
// chapter 9 will prune subtrees by prefix instead.
